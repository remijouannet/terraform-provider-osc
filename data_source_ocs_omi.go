...
func dataSourceOscOmi() *schema.Resource {
	return &schema.Resource{
		Read: dataSourceOscOmiRead,
...
// dataSourceOscOmiDescriptionRead performs the AMI lookup.
func dataSourceOscOmiRead(d *schema.ResourceData, meta interface{}) error {
	conn := meta.(*OSCClient).fcuconn
	...
	if filtersOk {
		params.Filters = buildOscDataSourceFilters(filters.(*schema.Set))
	}
	if ownersOk {
	...
	if len(filteredImages) > 1 {
		...
		if recent {
			image = mostRecentOmi(filteredImages)
		} else {
...
func mostRecentOmi(images []*ec2.Image) *ec2.Image {
	sortedImages := images
