...
func resourceOscOmi() *schema.Resource {
	// Our schema is shared also with aws_ami_copy and aws_ami_from_instance
	resourceSchema := resourceOscOmiCommonSchema(false)

	return &schema.Resource{
		Create: resourceOscOmiCreate,
		...
		// is created.
		Read:   resourceOscOmiRead,
		Update: resourceOscOmiUpdate,
		Delete: resourceOscOmiDelete,
	}
}

func resourceOscOmiCreate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*OSCClient).ec2conn
	...
	_, err = resourceOscOmiWaitForAvailable(id, client)
	if err != nil {
		return err
	}

	return resourceOscOmiUpdate(d, meta)
}

func resourceOscOmiRead(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*OSCClient).ec2conn
	id := d.Id()
	...
	if state == "pending" {
		...
		// the "Create" step.
		image, err = resourceOscOmiWaitForAvailable(id, client)
		if err != nil {
			return err
		}
		state = *image.State
...
func resourceOscOmiUpdate(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*OSCClient).ec2conn
	...
	return resourceOscOmiRead(d, meta)
}

func resourceOscOmiDelete(d *schema.ResourceData, meta interface{}) error {
	client := meta.(*OSCClient).ec2conn
...
func resourceOscOmiWaitForAvailable(id string, client *ec2.EC2) (*ec2.Image, error) {
...
func resourceOscOmiCommonSchema(computed bool) map[string]*schema.Schema {
